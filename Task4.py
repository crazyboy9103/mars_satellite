...
def f(x,t):
    xx=x[0] #in x-direction
    vx=x[1] #Relative Velocity in x-direction
    yy=x[2] #Relative position in y-direction
    vy=x[3] #Relative Velocity in y-direction
...
iv=sp.linspace(-2.5e4,-2e4,10) #Range of initial velocity of satellite in x-direction

for v in iv:
    xx0=[12*r, v, 12*r, -2500] #Initial condition for the satellite that escapes in front of Mars
    soln=spi.odeint(f,xx0,t)
    x=soln[:,0] #x-displacement
    xv=soln[:,1] #relative x-velocity
    y=soln[:,2] #y-displacement
    yv=soln[:,3] #relative y-velocity
    
    vs=((xv-mv)**2+yv**2)**0.5 #Magnitude of velocity of the satellite
